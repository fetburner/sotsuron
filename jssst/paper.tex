...
再帰関数や二つ組も扱えるような値呼びの関数型プログラミング言語をソース言語とした．
...
本研究では対象言語の意味を定めるために余帰納的大ステップ意味論を採用した．
これも証明手法の簡潔さや証明スクリプトの短さに寄与している．

\section{対象言語}
本研究におけるK正規化のソース言語の文法は\figurename\ref{eqn:mincaml-ast}のように表される．
前に述べた入出力、再帰関数や二つ組だけでなく，整数，真偽値，算術演算，ifやletといったプリミティブも持ち，関数は第一級である．
入出力専用の構文は存在しないが，自由変数に対する関数呼び出しは外部関数の呼び出しと見なされ，
