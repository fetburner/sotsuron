...
K-normalization is a program transformation that gives explicit names to all subexpressions using let bindings.
...
We adopted de Bruijn indices by comparison with several techniques to represent bindings.
}
%
...
しかしながら，入出力等の副作用があるような高階関数型プログラミング言語のコンパイラに対する形式的検証はあまり行われていない．
これは，(自由変数を持つ)関数を第一級オブジェクトとして扱えるような言語は複雑な束縛の操作を伴うほか，
無限に入出力を行うプログラムの意味論の形式化が自明でないといった問題のためである．

例えば，コンパイラの形式的検証の中でも良く知られた研究として，
...
CompCertでは関数定義の戦闘でしか変数を宣言できないため，束縛の操作は限定的となっている．
...
評価関係を式$e$，評価結果$v$，評価の際に行った入出力の(有限)列$t$との三項関係$e \Downarrow v~/~t$とした．
...
代入の代わりに環境を用いて対象言語の意味を定義する事もできるが．
...
(Coqでは単に$\texttt{CoInductive}$を用いている)．
\begin{figure}[tp]
	\begin{prooftree}
...
本研究では入出力を扱うため，評価関係を式$e$と，評価の際に行なわれた入出力の列$t$(無限列でもよい)との二項関係$e \Uparrow /~t$としている．
...
の等価性を議論できる利点はあるが，
入出力を含むような簡約と入出力を含まないような簡約(内部遷移)を区別して扱わなくてはならず，
...
そこで，対象言語の式を評価した結果得られる値と，K正規形を評価した結果得られる値との間に成り立つことが期待される
...
\end{figure}%
これを用いた\lemmaname\ref{lemma:eval-correctness}は
...
式$e$が関数抽象$\FIX.~e'$の場合に$e'$に値$v_i$が代入されても関係$v \rhd v'$が成り立つようにするためである．

なお，プログラムの等価性を証明するためによく用いられる手法の一つに論理関係\cite{Pierce:2004:ATT:1076265}が挙げられるが，
再帰関数を含むような言語や型無しの言語(ないし再帰型を含むような言語)では定義が煩雑になるため，本研究では採用を見送った．

\section{結論}\label{section:conclusion}
入出力等の副作用を持つ外部関数の呼び出しや再帰関数，高階関数，組を持つ値呼びの関数型プログラミング言語に対するK正規化を
定理証明支援系Coqを用いて形式的に検証できた．
無限の入出力も考慮しつつ様々な手法を比較検討して余帰納的大ステップ操作的意味論とde Bruijnインデックスを採用した結果，
比較的簡潔な証明が可能となった．

\section*{謝辞}
本稿を執筆するにあたりコメントを頂いた松田一孝先生に感謝します．
