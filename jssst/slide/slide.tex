...
\begin{frame}
	...
	\end{array} \]}
	\Large
	\only<2>{{\normalsize $(\lambda x.~e_2)~e_1$としても表現できるが}\\
		\alert<2>{K正規形を容易に表すため導入}}
	\only<3>{\alert<3>{自由変数の適用}(e.g. $\texttt{print\_int 42}$)\\
	...
	\begin{itemize}
		...
		\item captureを避ける必要がある
			\only<1>{\[\begin{array}{l}
				...
				\cancel{\overset{\alpha}{=}} \lambda z.~\alert{\LET~z=}x+y~\IN~z+\alert{z}
			\end{array}\]}
			\pause
			\[\begin{array}{l}
				K(\alert<3>{\lambda z.}~x+y+\alert<3>{z}) \\
				\overset{\alpha}{=} \alert<3>{\lambda z.}~\alert<2>{\LET~a=}x+y~\alert<2>{\IN~a}+\alert<3>{z}
			\end{array}\]
	\end{itemize}
\end{frame}
	...
	\begin{itemize}
		\item 内側から数えて何番目の束縛かで表現
			\begin{columns}
				...
				\begin{column}{0.4\textwidth}
					\tikz[remember picture]{\node(binding-by-name3){$\lambda x.~x~(\lambda y.~x~y)$};}
				\end{column}
				\begin{column}{0.025\textwidth}\end{column}
				\begin{column}{0.4\textwidth}
					\tikz[remember picture]{\node(de-Bruijn2){$\lambda.~0~(\lambda.~1~0)$};}
				\end{column}
			\end{columns}
		\item $\alpha$等価な式は文面上も同じ
			\begin{columns}
				...
				\begin{column}{0.4\textwidth}
					\begin{center}
						\tikz[remember picture]{\node(binding-by-name1){$\lambda x.\lambda y.~x$};}\\
						\tikz[remember picture]{\node(binding-by-name2){$\lambda a.\lambda b.~a$};}
					\end{center}
				\end{column}
				\begin{column}{0.4\textwidth}
					\tikz[remember picture]{\node(de-Bruijn1){$\lambda.\lambda.~1$};}
				\end{column}
			\end{columns}
	\end{itemize}
	\begin{tikzpicture}[remember picture, overlay]
		\draw[-implies, line width=1pt, double distance=4pt] (binding-by-name1.east) -- (de-Bruijn1.west);
		\draw[-implies, line width=1pt, double distance=4pt] (binding-by-name2.east) -- (de-Bruijn1.south west);
		\draw[-implies, line width=1pt, double distance=4pt] (binding-by-name3.east) -- (de-Bruijn2.west);
	\end{tikzpicture}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item ホスト言語の束縛で\\対象言語の束縛を表現\\
			$\lambda x.~x \equiv \texttt{Abs}~(\texttt{fun}~x \Rightarrow \texttt{Var}~x)$
			\begin{itemize}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.5\textwidth}
			\begin{itembox}[c]{\Large 有限回の入出力}
				\normalsize $(\FIX~f~x.~f~x)~(\texttt{print\_int}~\texttt{42})$
			\end{itembox}
		\end{column}
		\begin{column}{0.5\textwidth}
			\begin{itembox}[c]{\Large 無限回の入出力}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.5\textwidth}
			\begin{itembox}[c]{\Large 有限回の入出力}
				\normalsize $\begin{array}{l}
					(\FIX~f~x.~f~x)~(\texttt{print\_int}~\texttt{42}) \\
		...
		\begin{column}{0.5\textwidth}
			\begin{itembox}[c]{\Large 無限回の入出力}
				\normalsize $\begin{array}{l}
					(\FIX~f~x.~f~(\texttt{print\_int}~\texttt{42}))~\texttt{100} \\
					\Uparrow / \texttt{print\_int}(42)=0, \\
